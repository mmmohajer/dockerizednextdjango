...
from websocket.serializers import PublicChatRoomSerializer, PublicChatRoomMessageSerializer
...
class PublicChatConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def send_message(self, message):
        try:
            ...
            await create_public_room_chat_message(self.room, self.current_profile.user, message)
        except Exception as e:
            print(e)
            raise
    ...
    async def connected_user_count(self, event):
        ...
        await self.send_json({"msg_type": MSG_TYPE_CONNECTED_USER_COUNT, "connected_user_count": event["connected_user_count"]})

    async def join_room(self, room_id):
        try:
            room = await get_room_or_none(room_id)
            if room:
                data = PublicChatRoomSerializer(room).data
                await self.channel_layer.group_add(room.group_name, self.channel_name)
                await connect_user(room, self.current_profile.user)
                await self.send_json({"join": data})
                num_of_connected_users = await get_num_connected_users(room)
                await self.channel_layer.group_send(room.group_name, {"type": "connected.user.count", "connected_user_count": num_of_connected_users})
    ...
    async def leave_room(self):
        try:
            if self.room:
                ...
                await disconnect_user(self.room, self.current_profile.user)
                await self.send_json({"left": self.room_serializable})
                num_of_connected_users = await get_num_connected_users(self.room)
                await self.channel_layer.group_send(self.room.group_name, {"type": "connected.user.count", "connected_user_count": num_of_connected_users})
                self.room = None
            else:
...
def get_room_or_none(room_id):
    qs = get_object_or_404(PublicChatRoomModel, id=room_id)
    return qs
...
def create_public_room_chat_message(room, user, message):
    return PublicChatRoomMessageModel.objects.create(user=user, room=room, message=message)


@database_sync_to_async
def get_room_chat_messages(room, page_number):
    DEFAULT_ROOM_CHAT_MESSAGE_PAGE_SIZE = 10
    qs = PublicChatRoomMessageModel.objects.by_room(room)
    ...
    if page_number <= p.num_pages:
        ...
        serializer = PublicChatRoomMessageSerializer(current_page_qs, many=True)
